...
let rec findSolution (endpos: int * int) (field: SnowField) (time: int) (currentlyAt: Set<TimePos>) : int =
    let foundEnd =
        currentlyAt
    ...
    if foundEnd then
        ...
        let next (t, x, y) =
            let t = (time + 1) % (relevantTime input + 1)
        ...
        let nexts =
            ...
            |> List.collect next
            |> Set.ofList
            |> Set.intersect field.Field 

        let field = field.Field |> Set.filter (fun pos -> nexts.Contains pos |> not)
        1 + findSolution endpos (SnowField field)  (time + 1) nexts
