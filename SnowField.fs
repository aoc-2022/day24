...
type SnowField(field: Set<int * int * int>, timespan: int) =
    member this.Field = field
    member this.Timespan = timespan

    member this.Remove(remove: Set<TimePos>) =
        let field = field |> Set.filter (fun p -> remove.Contains p |> not)
        SnowField(field, timespan)
...
let initialSnowField (input: Input) : SnowField =
    ...
    SnowField(field, (relevantTime input))
